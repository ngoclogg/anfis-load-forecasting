...
\\
Trong đề tài này, hệ thống không chỉ dự báo một đầu ra chung mà tách thành hai horizon theo số giờ dự báo trước. Gọi $h \in \{1, 24\}$ là khoảng cách dự báo, mô hình cần học ánh xạ:

\begin{equation}
  \hat{y}_{t+h} = f_h(\mathbf{x}_t), \qquad h \in \{1, 24\}
\end{equation}
\\
Trong đó $\mathbf{x}_t$ là vector đặc trưng tại thời điểm $t$, $f_h$ là mô hình tương ứng với horizon $h$, và $\hat{y}_{t+h}$ là phụ tải dự báo tại thời điểm $t+h$. Khi $h=1$, bài toán tương ứng với dự báo 1 giờ tới và nhãn \texttt{target\_1h}; khi $h=24$, bài toán tương ứng với dự báo 24 giờ tới và nhãn \texttt{target\_24h}. Mốc 1 giờ tới phản ánh khả năng dự báo ngắn hạn gần thời gian thực, còn mốc 24 giờ tới phản ánh khả năng nắm bắt chu kỳ ngày và hỗ trợ lập kế hoạch cho ngày kế tiếp.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{5.2cm} p{7.9cm}}
    ...
    \texttt{load\_lag\_1} & Phụ tải của một giờ trước, giúp mô hình nắm xu hướng rất gần trước thời điểm dự báo. \\
...
\\
Với một horizon $h \in \{1, 24\}$, luật Sugeno thứ $i$ có dạng:
