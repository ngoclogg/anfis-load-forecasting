...
\begin{equation}
  MPE = \frac{100}{M}\sum_{t=1}^{M}\frac{y_t^{(h)} - \hat{y}_t^{(h)}}{y_t^{(h)}}
\end{equation}
\\
MAPE biểu diễn sai số trung bình theo phần trăm, còn MPE cho biết xu hướng dự báo lệch cao hay thấp. Khi dùng hai độ đo này cần đảm bảo $y_t^{(h)}$ khác 0 hoặc có cơ chế xử lý mẫu có giá trị rất nhỏ.
