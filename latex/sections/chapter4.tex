...
\begin{table}[H]
  ...
  \begin{tabular}{p{4.4cm} p{8.8cm}}
    ...
    \midrule
    Đặc trưng đầu vào & \texttt{apparent\_temperature}, \texttt{humidity}, \texttt{hour\_sin}, \texttt{hour\_cos}, \texttt{occupancy\_level}, \texttt{load\_lag\_1}, \texttt{load\_lag\_24}. \\
...
Mô hình được triển khai theo kiến trúc ANFIS 5 lớp với hàm thuộc Gaussian và hệ quả Sugeno bậc nhất. Trong thực nghiệm cuối cùng, mô hình sử dụng 7 biến đầu vào Core, 2 hàm thuộc cho mỗi biến và lưới luật đầy đủ.
...
\end{equation}
Các tâm $c_{ik}$ được khởi tạo từ phân vị của tập huấn luyện đã chuẩn hóa. Với hai hàm thuộc, mô hình tạo hai vùng đại diện cho mức thấp và cao của từng đặc trưng, giúp kiểm soát số luật mờ sau khi bổ sung \texttt{load\_lag\_1}. Tham số $\sigma_{ik}$ được chọn đủ lớn để tránh hàm thuộc quá hẹp, giúp quá trình suy diễn ổn định hơn.

\section{Sinh luật mờ}

Do số lượng đặc trưng Core được giới hạn ở 7 biến, dự án sử dụng lưới luật đầy đủ. Mỗi luật là một tổ hợp giữa các hàm thuộc của tất cả biến đầu vào. Nếu mỗi biến có 2 tập mờ, tổng số luật là:

\begin{equation}
  N_{\text{rules}} = 2^7 = 128
\end{equation}
Trong cấu hình cuối cùng, 7 biến đầu vào Core Features gồm:
\begin{itemize}[leftmargin=1.2cm, itemsep=0pt]
  \item \texttt{apparent\_temperature}, \texttt{humidity};
  \item \texttt{hour\_sin}, \texttt{hour\_cos};
  \item \texttt{occupancy\_level}, \texttt{load\_lag\_1}, \texttt{load\_lag\_24}.
\end{itemize}
Mỗi biến sử dụng 2 hàm thuộc Gaussian, còn luật mờ được sinh theo phương pháp grid partition. Cách thiết lập này giúp mô hình giữ được khả năng biểu diễn quan hệ phi tuyến giữa các đặc trưng, đồng thời hạn chế số luật mờ tăng quá lớn; nếu tăng số biến đầu vào hoặc số hàm thuộc trên mỗi biến, số luật sẽ tăng theo cấp số nhân.

Luật mờ thứ $i$ có dạng tổng quát:

\begin{equation}
  R_i: \text{ IF } x_1 \text{ is } A_{i1}, \ldots, x_7 \text{ is } A_{i7}
  \text{ THEN } f_i^{(h)} = p_{i0}^{(h)} + \sum_{k=1}^{7}p_{ik}^{(h)}x_k
\end{equation}
Trong đó $h \in \{1, 24\}$ là horizon dự báo theo đơn vị giờ. Mỗi horizon có bộ tham số hệ quả riêng vì \texttt{target\_1h} và \texttt{target\_24h} mô tả hai bài toán khác nhau.
Mức kích hoạt của luật được tính bằng tích các độ thuộc:
\begin{equation}
  w_i = \prod_{k=1}^{7}\mu_{A_{ik}}(x_k)
\end{equation}
...
\end{enumerate}
Cách này phù hợp với phạm vi bài tập lớn vì ổn định, dễ kiểm soát và cho phép lưu lại mô hình dưới dạng các ma trận NumPy. Ngoài pipeline huấn luyện chính, dự án có thêm script kiểm chứng riêng bước fit hệ quả Sugeno trên Core validation split. Script này nạp dữ liệu bằng \texttt{load\_core\_data}, chia dữ liệu bằng \texttt{split\_train\_val\_test}, khởi tạo membership từ \texttt{train\_fit}, fit hệ quả bằng Ridge Least Squares và lưu \texttt{model.npz} cùng \texttt{training\_log.csv} trong thư mục kiểm chứng riêng.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{0.94\textwidth}@{}}
    ...
    \begin{minipage}{0.92\textwidth}
      ...
      \hspace*{3.1em}hour\_cos, occupancy\_level, load\_lag\_1, load\_lag\_24]\\[0.35em]
      ...
      \hspace*{1.5em}model = ANFIS(n\_mfs=2, mf=Gaussian, rules=2\^{}7)\\
